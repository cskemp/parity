...
# function for frequentist regression analysis
run_glmmTMB <- function(dstats, domainname) {
  ...
  if (domainname %in% paste0("m_", kinship_classes)) {
     ...
     if (domainname %in% c("HIDElocational (other)")) {
       rtype <- "lang"
...
# function for Bayesian regression analysis
run_brms <- function(dstats, domainname) {
  ...
  } else if (domainname == "HIDElocational (other)") {
    rtype <- "lang"
     m1 <- brm( parity ~ (1|glottocode), data = dstats, family = bernoulli(), chains = nchain, iter = niter, control = b_con ) 
  } else if(max_lang == 1 ) {
    rtype <- "langfamily"
